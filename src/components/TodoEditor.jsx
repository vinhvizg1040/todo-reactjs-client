...
const TodoEditor = (props) => {
    ...
    function handleInputChange(event) {
        ...
        setFormData({ ...formData, [name]: value }); // Đồng bộ giá trị của trường input với biến title hoặc description
    }
        ...
        <div id='todoEdit' className='h-full w-full flex flex-col items-center bg-[#202020]'>
            <div className='w-3/5 flex flex-col justify-between pt-10'>
                <div className='h-24 w-full flex flex-col items-center'>
                    ...
                    <button onClick={handleClick} className='w-full p-2 flex justify-items-start group'>
                        <div className='flex'>
                            <span className='group-hover:bg-red-500 group-hover:rounded-full h-5 w-5 p-1'>
                                <svg className='fill-red-500 group-hover:fill-white' width="12" height="12" viewBox='0 0 12 12'><path d="M 6 6 V 0.5 A 0.5 0.5 90 0 1 7 0.5 V 6 H 12.5 A 0.5 0.5 90 1 1 12.5 7 H 7 V 12.5 A 0.5 0.5 90 1 1 6 12.5 V 7 H 0.5 A 0.5 0.5 90 0 1 0.5 6 H 6 Z" fill="current" fillRule="evenodd"></path></svg>
                            </span>
                ...
                {showForm && (
                    <div className='h-40 w-full border rounded-lg border-white/20 pt-1'>
                        <input name="title" value={formData.title} onChange={handleInputChange} className='h-8 w-full bg-[#202020] focus:outline-none text-gray-500 text-sm font-semibold p-2 text-start' placeholder="Title"></input>
                        <input name="description" value={formData.description} onChange={handleInputChange} className='h-6 w-full bg-[#202020] focus:outline-none text-gray-500 text-xs font-medium p-2 text-start' placeholder="Description"></input>
                        <div className='bg-[#202020] h-12 w-full border-b border-white/20 px-1 py-2 flex justify-start'>
                            <DatePicker />

                            <button className='p-1 -ml-1 mr-1 h-7 flex items-center border border-white/30 rounded-lg text-gray-500 text-xs font-medium'>
