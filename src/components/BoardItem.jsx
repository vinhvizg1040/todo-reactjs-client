...
const BoardItem = (props) => {
    ...
    const onRemoveBoard = async () => {
        await instance.delete('/board/deleteBoardbyId',
            {
                data: { board_id: props.board.board_id }
            })
    ...
    return (
        <div className='flex py-1 px-4'>
            ...
            {editing ?
                (
                    <button className='flex px-2 py-1 w-full'>
                        <input className='bg-[#363636] leading-5 pl-2 font-extralight' type="text" value={newName} onChange={handleChange} />
                    </button>
                )
                : (
